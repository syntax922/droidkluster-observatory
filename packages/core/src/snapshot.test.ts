...
function stateWithActiveReview() {
  ...
  s = reduce(
    s,
    {
      kind: "event",
      id: "a",
      subject: "gh.event.exampleproj.pr.opened.5",
      ts: T0,
      payload: {
        action: "opened",
  ...
  s = reduce(
    s,
    {
      kind: "event",
      id: "b",
      subject: "gh.event.exampleproj.pr.review_started.5",
      ts: T0,
      payload: {
  ...
  it("a completed chain that receives a reopen becomes active again", () => {
    ...
    s = reduce(
      s,
      {
        kind: "event",
        id: "b",
        subject: "gh.event.exampleproj.pr.closed.7",
    ...
    s = reduce(
      s,
      {
        kind: "event",
        id: "c",
        subject: "gh.event.exampleproj.pr.reopened.7",
  ...
  it("carries last_action_at through from the reducer's idle transition", () => {
    ...
    s = reduce(
      s,
      {
        kind: "event",
        id: "c",
        subject: "gh.event.exampleproj.pull_request_review.submitted.5",
