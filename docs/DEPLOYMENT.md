...
and the projector (containerized, deployed from the private Kluster
GitOps repo).
...
3. **Manage R2 API tokens** → **Create API token**.
   ...
   - Save the resulting Access Key ID, Secret Access Key, and Account ID —
     ...
     Kluster secret store, not in this repo.
...
**The k8s manifests themselves live in the private Kluster repo, not
