...
the private `homesecretary-components` / Kluster repos and is out of scope
...
- **Private repository content.** `homesecretary-components` (worker
  source, review/CI-diagnosis logic, NATS subject wiring in full) and the
  Kluster GitOps repo (cluster manifests, secret references) are private.
...
| **Cloudflare Pages / the static site** | Defacement — the ability to serve arbitrary JS to visitor browsers. The site holds no credentials, makes only unauthenticated `GET`s, and has no write path back to the bucket or the cluster. Worst case is a malicious page, not a foothold anywhere else. |
...
- **NATS subject naming is visible in public source (accepted).** The
  ...
  `dungeonadventures.event.merge_decision.reached.*`,
  `gh.event.dungeonadventures.*` — which reveals the fleet's internal
